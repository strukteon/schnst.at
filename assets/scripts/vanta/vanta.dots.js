...
class Effect extends VantaBase {
  ...
  onInit() {
    var camera = this.camera = new PerspectiveCamera(50, this.width / this.height, 0.1, 5000)
    camera.position.x = 0
    ...
    var starsGeometry = this.starsGeometry = new BufferGeometry()
    ...
    if (this.options.showLines) {
      var material = new LineBasicMaterial( { color: this.options.color2 } );
      var linesGeo = new BufferGeometry()
      const points = []
      for (i = 0; i < 200; i ++) {
        var f1 = rn(40,60)
        var f2 = f1 + rn(12,20)
